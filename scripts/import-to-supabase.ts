      ...
      batch.map((record: any) =>
        prisma.appointment.create({
          data: {
            ...
            apptSpId: record.ApptSP_ID ? BigInt(record.ApptSP_ID) : null,
            apptTypeId: record.Appt_TypeID || null,
            apptDate: new Date(record.Appt_Date.split('.')[0]),
            ...
            created: new Date(record.Created.split('.')[0]),
            updated: new Date(record.Updated.split('.')[0]),
